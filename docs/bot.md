...
bot/admin_bot.py  закрытый интерфейс создания рассылок
bot/broadcasts.py аудитория, история и доставка через основной бот
```

## Админ-бот и рассылки

Отдельный админ-бот принимает одно сообщение с текстом или материалом, позволяет
добавить до восьми URL-кнопок, показывает предпросмотр и умеет отправить тест только
тому администратору, который нажал кнопку. Массовая отправка начинается лишь после ввода точной
фразы `РАЗОСЛАТЬ N`, где `N` — актуальное число получателей.

В аудиторию входят только активные записи `telegram_bot`, у которых есть событие
`bot_start`. Доставку выполняет основной бот из `TELEGRAM_BOT_TOKEN`: Telegram не
разрешает новому боту первым писать пользователям, которые запускали другой бот.

Переменные окружения:

```bash
ADMIN_TELEGRAM_BOT_TOKEN=...
ADMIN_BOT_OWNER_IDS=123456789
ADMIN_BOT_SEND_INTERVAL=0.05
```

Команды владельца:

- `/admins` — список активных администраторов;
- `/add_admin TELEGRAM_ID Имя` — дать заказчику доступ;
- `/remove_admin TELEGRAM_ID` — отозвать доступ (владельца удалить нельзя).

Статусы каждой рассылки и каждого получателя сохраняются в `broadcasts` и
`broadcast_deliveries`, поэтому подтверждённая рассылка продолжится после перезапуска.

Админ-бот также напоминает всем активным администраторам выгрузить свежие данные
из TikTok. Отсчёт начат 13 июля 2026 года в 15:00 МСК: первое напоминание придёт
16 июля в 15:00 МСК, следующие — каждые три дня. Успешная отправка фиксируется в
`events`, поэтому перезапуск сервиса не создаёт дубль.
