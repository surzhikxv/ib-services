...
- Мёртвый BotHelp-коннектор на новый ABC НЕ мигрируем (он больше не запускается). Папка `bothelp/` остаётся только как структурный образец раскладки; ABC валидируется новыми контент-коннекторами и их тестами.
...
`order_id` (= `Payment.external_id`) даёт общую идентичность Event и Payment. **BotHelp больше не источник:** воронка полностью на нашем коде, `bothelp sync` не запускаем и коннектор не расширяем. Бот — единственный источник событий воронки. Исторические строки BotHelp, уже импортированные в Phase 1 (если они есть в озере), остаются как есть; новые события пишет только бот (`source_system='telegram_bot'`).
