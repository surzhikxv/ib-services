...
**Ветка-предшественник:** `deploy/bot-prodamus` (воронка BotHelp → наш aiogram, оплаты Prodamus).
...
Сейчас озеро наполнено только со стороны **воронки/продаж** (коннектор BotHelp). Половина «контент → бот → оплата» — со стороны **трафика и контента** (TikTok, VK, Instagram, Telegram-канал, YouTube) — пустая. Без неё ИИ-аналитик видит только хвост (продажи), но не голову (какой контент даёт трафик). Эта фаза наполняет вторую половину и доводит фундамент, чтобы события воронки писались напрямую из нашего бота, а ИИ-аналитик заработал вживую через офшорный релей.
...
### 4.1 Базовый класс
`kontur/connectors/base.py` сейчас — пустой скелет (`name` + абстрактный `sync(session_factory)`), и BotHelp его даже **не наследует** (там свободная функция `sync_bothelp(client, …, bot_referral)`). Делаем настоящий ABC по template-method:
...
- BotHelp ретрофитим на новую базу через тонкий адаптер (существующие тесты `test_sync` остаются зелёными).

### 4.2 Раскладка каждого коннектора (зеркало `bothelp/`)
...
- **Таймзоны:** `_ts()` от BotHelp = unix UTC; YouTube отдаёт ISO-строки, VK — unix (день по Москве). Каждый `mapping.py` парсит в tz-aware UTC, иначе `snapshot_date` уезжает на день у полуночи.

### 4.5 Сквозные общие узлы
- **Канонический нормализатор UTM** (один общий helper, не per-connector): единый словарь ключей (`utm_source`→`utmSource`), та же сортировка/регистр/формат, что в `sync.py:107`. И BotHelp, и контент-коннекторы зовут его. Тест: контент-Source и subscriber-Source из одного логического UTM дают идентичный `code`. Иначе атрибуция «контент→воронка» молча даёт ноль джойнов.
...
- **Слой rate-limit/backoff в `client.py`** (BotHelp не нужен был): VK error 6, YouTube квоты 10k юнитов/день, Telegram FLOOD_WAIT.
...
**Данные:** Channel(platform='telegram_channel', external_id='-100…' — НЕ путать с bothelp-Channel platform='telegram'/referral; разные строки, не сливаются автоматически), Content (посты, `.views/.forwards` best-effort), content_metric (per-post views по дням; источник истины — `getMessageStats.views_graph` где `can_view_stats=true`), event роста подписчиков.
...
`order_id` (= `Payment.external_id`) даёт общую идентичность Event и Payment. dedup_key уникален per source_system → `telegram_bot` и `bothelp` не коллидируют. **BotHelp-коннектор → исторический бэкфилл:** гонять `bothelp sync` только для до-cutover истории, дальше не планировать; сверка эпох по `Subscriber.tg_user_id`.
...
**Операционные вопросы (не блокируют старт):** cutover-таймстамп для границы BotHelp-бэкфилла; TG user-id владельца для `CABINET_OWNER_IDS`; подтверждение, что «по тегу» = тарифные сегменты (иначе дописать запись тегов в живую воронку).
