...
class ContentMetric(Base, TimestampMixin):
    ...
    Одна строка на контент/UTC-день; повторный прогон В ТОТ ЖЕ день ПЕРЕЗАПИСЫВАЕТ её
    (метрики растут в течение дня — берём последнее чтение). Коннекторы пишут ОБА хранилища:
